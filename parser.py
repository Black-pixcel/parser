...
class Get_Logs:
    ...
    def insert_into_database(self, *, clear_table = False):
        ...
        # Проверяем существование в базе данных таблицы для логов
        ...
        if check_table == 0:
            ...
            # Если  в функцию передан именованный аргумент преварительной очистки таблицы, то очищаем её
            if clear_table == True:
                cur.execute('truncate table logs')
                print('Таблица очищена')

        # Вставляем логи в таблицу
...
if __name__ == "__main__":
    # Вводим дату для выгрузки логов
    log_date = get_date()
    # Получаем логи
    logs = Get_Logs(log_date)
    # Сортируем логи
    logs.sort()
    # Вставляем логи в базу данных
